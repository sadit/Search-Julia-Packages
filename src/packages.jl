...
"""
    search_pkgs(X, q;
        ...
        expansion::Int=10,
        wname::Float32=1f0,
        wreadme::Float32=0.3333f0
    )

Search for packages in the `X` database and index

"""
function search_pkgs(X, q;
        k::Int=3,
        expansion::Int=10,
        wname=1f0,
        wreadme=0.3333f0
    )
    wname = convert(Float32, wname) 
    wreadme = convert(Float32, wreadme) 
    Rdesc = KnnResult(10k)
